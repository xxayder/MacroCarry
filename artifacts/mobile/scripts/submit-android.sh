#!/bin/bash
# submit-android.sh
# Submits the latest EAS-built AAB to Google Play (skips the build step).
...
# Catches permission problems before spending time on the submit step.
...
case "$EDIT_HTTP" in
  ...
  403)
    ...
    echo "'Release manager' permission for this app:"
    echo "  https://play.google.com/console/developers/api-access"
    exit 1
    ;;
  404)
